...
- The NIPTeR upstream reference is `../../duckhts/.sync/NIPTeR/` (read-only algorithm reference).
- `reticulate` remains an optional dependency used only for conformance tests and optional helpers.

## Source Layout Expectations

- `R/convert.R` — shared BAM/CRAM binning engine (`bam_convert()`, `bam_convert_bed()`); the central read-counting layer used by both WisecondorX and NIPTeR layers.
- `R/nipter_bin.R` — NIPTeR binning layer (`nipter_bin_bam()`, `nipter_bin_bam_bed()`); produces `NIPTeRSample` objects and bgzipped BED output.
- `R/nipter_gc.R` _(planned)_ — `nipter_gc_correct()` using `rduckhts_fasta_nuc()` for GC content; no bundled correction tables.
- `R/nipter_score.R` _(planned)_ — `nipter_z_score()`, `nipter_ncv_score()`, `nipter_regression()`.
- `R/nipter_chi.R` _(planned)_ — `nipter_chi_correct()`.
- `R/nipter_control.R` _(planned)_ — `nipter_as_control_group()` and related group management.
- `R/wisecondorx_cli.R` — CLI wrappers (`wisecondorx_convert()`, `wisecondorx_newref()`, `wisecondorx_predict()`).
- `R/npz.R` — `bam_convert_npz()` for WisecondorX-compatible NPZ output.
- `R/aaa.R` — SRA metadata helpers.
- `inst/tinytest/` — unit tests (one file per feature family).
- `inst/extdata/` — synthetic BAM/CRAM fixtures generated by `scripts/make_fixtures.sh`.
...
- Four-column WisecondorX BED: `chrom`, `start`, `end`, `count` (written by `bam_convert_bed()`).
- Five-column NIPTeR BED: `chrom`, `start`, `end`, `count`, `corrected_count` (written by `nipter_bin_bam_bed()`).
...
- These files are queryable directly with `DuckDB` using the `read_csv()` or `hts_read_bgzf_bed()` functions.
...
- Improper pairs (`is_paired AND NOT is_proper`) are invisible to both larp and larp2. This filter is applied only for `rmdup = "streaming"` — it is intrinsic to the WisecondorX algorithm, not a user flag option.
...
The conformance script achieves exact bin-for-bin match on HG00106.chrom11 (25,115 non-zero bins, 0 mismatches).

## Flag Filtering API

`bam_convert()` and `bam_convert_bed()` expose samtools-style `-f`/`-F` flag filtering:
...
- Use the duckhts UDFs `sam_flag_bits(flag)` and `sam_flag_has(flag, bit)` to inspect SAM flags in SQL if needed.

The NIPTeR layer (`nipter_bin_bam()`) uses `require_flags = 0L, exclude_flags = 0L` by default (all mapped reads, matching NIPTeR's `scanBam` defaults).

## NIPTeR Layer

The NIPTeR port follows the same rewrites.bio principles as the WisecondorX port: exact emulation, credit original authors, AI transparency. Key differences from the original:

- `Rsamtools::scanBam()` is replaced by `bam_convert()` (DuckDB/Rduckhts SQL engine).
- GC content will be derived from the FASTA reference via `rduckhts_fasta_nuc()` — no bundled correction tables.
...
2. When upstream WisecondorX behavior is unclear, consult `../../duckhts/.sync/WisecondorX/` (algorithm reference only). Do NOT look for `inst/python/` — it has been removed.
3. When upstream NIPTeR behavior is unclear, consult `../../duckhts/.sync/NIPTeR/`.
4. For conformance testing, use the official `wisecondorx` bioconda package via `condathis`, not any vendored copy.
5. Preserve CRAN-friendly package behavior. Do not make package loading depend on a preconfigured external Python installation.
6. Keep package runtime logic and conformance tooling separate.
7. Keep the WisecondorX and NIPTeR layers in **separate files** with strict naming conventions (`nipter_*.R` vs. `wisecondorx_*.R`). The shared engine is in `convert.R`.
...
- Use make targets: `make rd` (roxygen2), `make test` (tinytest), `make readme` (render README), `make fixtures` (synthetic BAMs).
...
- Do not make package loading depend on `reticulate` or Python availability.
...
- The `rmdup` parameter in `bam_convert()` must support: `"streaming"` (WisecondorX larp/larp2), `"none"` (NIPTeR default / `--normdup`), `"flag"` (Picard/sambamba pre-marked BAMs).
...
- NIPTeR authors Dirk de Weerd and Lennart Johansson are listed as `cph` in `DESCRIPTION` in accordance with the rewrites.bio attribution principle.
- WisecondorX authors are listed as `ctb`.
- Any new upstream algorithm ported must have its original authors credited in `DESCRIPTION` with an appropriate role (`cph` for copyright holders of ported code).

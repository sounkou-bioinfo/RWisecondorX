...
#' across chromosomes 1-22, X, and Y, with no MAPQ filter and no pair-status
#' filter (all mapped reads are counted), matching the Rsamtools `scanBam`
#' defaults used by the original.
#'
#' Internally calls [bam_convert()] with `mapq = 0L`,
#' `filter_improper_pairs = FALSE`, and the caller-supplied `rmdup`. The result
#' is reshaped into a `NIPTeRSample` object whose structure parallels
#' NIPTeR's `NIPTSample`: autosomal reads as a chromosome-by-bin matrix and sex
#' chromosome reads as a separate two-row matrix.
#'
...
#'   or `"flag"` (for BAMs already processed by Picard / sambamba). NIPTeR does
#'   not perform streaming deduplication.
...
nipter_bin_bam_bed <- function(bam,
  ...
  sample <- nipter_bin_bam(bam, binsize = binsize, rmdup = rmdup,

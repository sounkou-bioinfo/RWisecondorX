...
kernel bundled in `Rduckhts`, with no Python dependency.
...
index, using `rduckhts_bgzip` and `rduckhts_tabix_index` from `Rduckhts`
...
#>   bed_created index_created   bed_size_kb 
#>           1.0           1.0           4.3
...
reads a 5-column NIPTeR BED.gz into a `NIPTeRSample` object compatible
with all NIPTeR statistical functions. Column count is auto-detected (5
= CombinedStrands, 9 = SeparatedStrands).
...
The example below generates a 50-sample synthetic cohort (35 euploid
female, 12 euploid male, 1×T21, 1×T18, 1×T13), builds a reference, and
verifies that a trisomy sample is correctly detected.
...
#> [1] chr    start  end    ratio  zscore type  
...
[`rwisecondorx_newref()`](https://sounkou-bioinfo.github.io/RWisecondorX/reference/rwisecondorx_newref.md)
also accepts a directory of 4-column BED.gz files produced by
[`bam_convert_bed()`](https://sounkou-bioinfo.github.io/RWisecondorX/reference/bam_convert_bed.md)
via the `bed_dir` parameter. All matching files are loaded in a single
DuckDB pass — no in-memory sample list needed:
...
[`rwisecondorx_predict()`](https://sounkou-bioinfo.github.io/RWisecondorX/reference/rwisecondorx_predict.md)
supports a `beta` parameter for purity-based aberration calling,
appropriate for somatic CNV analysis where tumour purity is known. When
`beta` is supplied, ratio cutoffs replace Z-score thresholds:
...
creates a 50-sample synthetic BAM cohort using compressed chromosome
lengths (100bp = 1 genomic 100kb bin), so each BAM is ~435KB. The cohort
contains euploid females and males plus three trisomy samples (T21, T18,
...
chromosomal fraction distance (Rcpp+OpenMP accelerated).
...
(default, matching NIPTeR) and bin-weight normalisation. For cohort
workflows,
...
computes the chromosomal fraction Z-score for a focus chromosome. A
Z-score above 3 is the conventional threshold for trisomy.
...
computes the normalised chromosome value, searching for the denominator
chromosome set that minimises control-group variance before computing
the Z-score. More robust than the standard Z-score when the control
group is small.
...
consensus (ties → female, the conservative choice for NIPT).
...
[`VALIDATION.md`](https://sounkou-bioinfo.github.io/RWisecondorX/VALIDATION.md)
for a full account of what has been validated, on what data, against
which upstream versions, and what remains unvalidated.

**Summary of current status:**

- [`bam_convert()`](https://sounkou-bioinfo.github.io/RWisecondorX/reference/bam_convert.md):
  bin-for-bin identical to Python WisecondorX on `HG00106.chrom11` (0
  mismatches, 25,115 bins).
...
- Trisomy detection: T21/T18/T13 all correctly called in
  `test_cohort_pipeline.R` and `test_wisecondorx_e2e.R` using synthetic
  cohort.
- `rwisecondorx_newref/predict` segment and aberration comparison
  against upstream Python: **pending real-data run** (run
...
roxygen2 comments, `make fixtures` to rebuild the bundled test data, and

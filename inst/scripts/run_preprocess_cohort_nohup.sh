...
Environment:
  ...
  PACKAGE_LIB                  R library path containing the current RWisecondorX build
  ...
  NIPTER_GC_INCLUDE_SEX        1 to GC-correct X/Y before NIPTeR BED export [default: 0]
  ...
  TABIX_METADATA               1 to write ##RWX_<key>=<value> metadata lines [default: 1]
...
THREADS="${1:-20}"
DEFAULT_SOURCE_MANIFEST="/mnt/data/BixCTF/NiptSeqNeo/all_bam_list_last_100_days.txt"
...
DEFAULT_PACKAGE_LIB="${REPO_ROOT}/.rlib"
...
PACKAGE_LIB="${PACKAGE_LIB:-${DEFAULT_PACKAGE_LIB}}"
SEQFF_TSV="${SEQFF_TSV:-}"
...
TABIX_METADATA="${TABIX_METADATA:-1}"
...
TABIX_METADATA_LOGICAL="$(bool_to_r_logical "${TABIX_METADATA}")"
...
if [[ ! -d "${PACKAGE_LIB}" ]]; then
  echo "PACKAGE_LIB does not exist: ${PACKAGE_LIB}" >&2
  exit 1
fi
...
echo "  TABIX_METADATA=${TABIX_METADATA_LOGICAL}"
...
cmd=(
  ...
  --tabix-metadata="${TABIX_METADATA_LOGICAL}"
